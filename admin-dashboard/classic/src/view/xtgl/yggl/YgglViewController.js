Ext.define('Admin.view.xtgl.yggl.YgglViewController', {
		...
		Ext.MessageBox.confirm('提示', '确定要进行删除操作吗？',
  			function(btn, text){
            	if(btn=='yes'){
					...
					store.remove(record);//DELETE //http://localhost:8081/yggl/112
					//store.sync();
				}
        	}
        , this);
	...
	quickSearch:function(btn){
		...
		Ext.apply(store.proxy.extraParams, {userId:""});
		Ext.apply(store.proxy.extraParams, {userName:""});
	...
	submitAddForm:function(btn){
		...
		record.set(values);
		record.save();
		Ext.data.StoreManager.lookup('ygglGridStore').load();
		win.close();
	},
	...
	submitEditForm:function(btn){
		var win    = btn.up('window');
		var store = Ext.data.StoreManager.lookup('ygglGridStore');

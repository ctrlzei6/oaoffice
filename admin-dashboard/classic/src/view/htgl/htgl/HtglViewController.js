Ext.define('Admin.view.htgl.htgl.HtglViewController', {
		...
		Ext.MessageBox.confirm('提示', '确定要进行删除操作吗？',
  			function(btn, text){
            	if(btn=='yes'){
					...
					store.remove(record);//DELETE //http://localhost:8081/htgl/112
					//store.sync();
				}
        	}
        , this);
	...
	quickSearch:function(btn){
		...
		Ext.apply(store.proxy.extraParams, {compactId:""});
		Ext.apply(store.proxy.extraParams, {compactName:""});
		if(searchField==='compactId'){
			Ext.apply(store.proxy.extraParams, {compactId:searchValue});
		};
		if(searchField==='compactName'){
			Ext.apply(store.proxy.extraParams, {compactName:searchValue});
		}
	...
	submitAddForm:function(btn){
		...
		var record = Ext.create('Admin.model.glmodel.htgl.htgl.HtglModel');
		var values  =form.getValues();//获取form数据
		record.set(values);
		record.save();
		Ext.data.StoreManager.lookup('htglGridStore').load();
		win.close();
	},
	...
	submitEditForm:function(btn){
		var win    = btn.up('window');
		var store = Ext.data.StoreManager.lookup('htglGridStore');

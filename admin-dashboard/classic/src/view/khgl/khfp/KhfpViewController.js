Ext.define('Admin.view.khgl.khfp.KhfpViewController', {
    extend: 'Ext.app.ViewController',
	alias: 'controller.khfpViewController',
	
	//添加
	openAddWindow:function(grid, rowIndex, colIndex){
		var win = grid.up('khfp').add(Ext.widget('khfpAddWindow'));
		win.show();
	},
	khfpAddFormSubmit:function(btn){
	...
	openCheckWindow:function(grid, rowIndex, colIndex){
		...
		if(record){
			var win = grid.up('khfp').add(Ext.widget('khfpCheckWindow'));
			win.show();
	...
	openEditWindow:function(grid, rowIndex, colIndex){
		var record = grid.getStore().getAt(rowIndex);
		if(record){
			var win = grid.up('khfp').add(Ext.widget('khfpEditWindow'));
			win.show();
			win.down('form').getForm().loadRecord(record);
		}else{
			Ext.Msg.alert("警告","请选择一条数据进行操作！")
	...
	// 	toolbar.up('grid').up('khfp').add(Ext.widget('khfpSearchWindow')).show();
	// },
	...
	submitAddForm:function(btn){
		...
		var record = Ext.create('Admin.model.glmodel.khgl.khfp.KhfpModel');
		var values  =form.getValues();//获取form数据
		record.set(values);
		record.save();
		Ext.data.StoreManager.lookup('khfpGridStore').load();
		win.close();
	},
	...
	submitEditForm:function(btn){
		var win    = btn.up('window');
		var store = Ext.data.StoreManager.lookup('khfpGridStore');
